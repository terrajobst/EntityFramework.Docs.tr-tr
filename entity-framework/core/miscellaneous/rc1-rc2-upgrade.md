...
RC1 ve RC2 arasında "Entity Framework 7" iken "Entity Framework Core" olarak değiştiriyoruz. [Scott Hanselman tarafından bu postadaki](http://www.hanselman.com/blog/ASPNET5IsDeadIntroducingASPNETCore10AndNETCore10.aspx)değişikliğin nedenleri hakkında daha fazla bilgi edinebilirsiniz. Bu değişiklik nedeniyle, paket adlarımız sürümünden `EntityFramework.*` `Microsoft.EntityFrameworkCore.*` ve sürümlerimize `7.0.0-rc1-final` (ya da `1.0.0-preview1-final` araç `1.0.0-rc2-final` için) olarak değiştirildi.
...
Paket adlarıyla birlikte, ad alanları ' den `Microsoft.Data.Entity.*` `Microsoft.EntityFrameworkCore.*`' a değiştirilir. Bu değişikliği `using Microsoft.Data.Entity` ile `using Microsoft.EntityFrameworkCore`bir Bul/Değiştir ile işleyebilirsiniz.

## <a name="table-naming-convention-changes"></a>Tablo adlandırma kuralı değişiklikleri

RC2 'de yaptığımız önemli bir işlevsel değişiklik, belirli bir varlık için `DbSet<TEntity>` özelliğin adını yalnızca sınıf adı yerine, eşlendiği tablo adı olarak kullanmıştı. [İlgili duyuru sorununu](https://github.com/aspnet/Announcements/issues/167)bu değişiklik hakkında daha fazla bilgi edinebilirsiniz.
...
RC1 'de, uygulama hizmeti sağlayıcısına `Startup.ConfigureServices(...)`Entity Framework hizmetleri eklemek zorunda kaldık:
...
RC2 'de,, vb. çağrıları `AddEntityFramework()` `AddSqlServer()`kaldırabilirsiniz:
...
Bağlamını bağlama işlemi yapan bir `IServiceProvider` RC1 kodunuz varsa, bu artık ayrı bir oluşturucu parametresi yerine öğesine `DbContextOptions`taşınır. Hizmet `DbContextOptionsBuilder.UseInternalServiceProvider(...)` sağlayıcısını ayarlamak için kullanın.
...
Bir ASP.NET Core uygulamanız varsa ve uygulama hizmeti sağlayıcısından iç Hizmetleri çözümlemek istiyorsanız, bunu yapılandırmanıza olanak `AddDbContext` tanıyan aşırı yüklemesi vardır:
...
Daha önce ASP.NET 5 projeleri `dnx ef` için komutları kullandıysanız, bunlar artık `dotnet ef` komutlara taşınmıştır. Aynı komut söz dizimi hala geçerlidir. Sözdizimi bilgileri için `dotnet ef --help` kullanabilirsiniz.

DNX 'in .NET CLı tarafından değiştirilmeleri nedeniyle, komutların kayıtlı olduğu Yöntem RC2 'de değiştirilmiştir. Komutlar artık içindeki `tools` `project.json`bir bölüme kaydedilir:
...
> [!TIP]  
> Visual Studio kullanıyorsanız artık ASP.NET Core projelerine yönelik EF komutlarını çalıştırmak için Package Manager konsolunu kullanabilirsiniz (Bu, RC1 'de desteklenmez). Bunu yapmak `tools` `project.json` için bölümünde komutları kaydetmeniz gerekir.
